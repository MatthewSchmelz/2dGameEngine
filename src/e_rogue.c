...
void rogue_update(Entity* self);
void rogue_free(Entity* self);
...
Entity* rogue_new() {
	...
	self->think = rogue_think;
	self->update = rogue_update;
	self->free = rogue_free;
	self->hitbox = gfc_circle(self->position.x, self->position.y, 400);
	self->team = 1;
	self->health = 1;
	self->pursue = rogue_pursue;
	self->draw = rogue_draw;
};
...
void rogue_pursue(Entity* self, Entity* target) {
	...
	*/
	...
	if (self->position.x < loc.x) dir.x = 1.5;
	if (self->position.y < loc.y) dir.y = 1.5;
	if (self->position.x > loc.x) dir.x = -1.5;
	if (self->position.y > loc.y) dir.y = -1.5;
	vector2d_normalize(&dir);
