...
void tank_free(Entity* self);
void tank_pursue(Entity* self, Entity* target);
void tank_draw(Entity* self);
...
Entity* tank_new() {
	...
	*/
	...
	wjson = sj_object_get_value(json, "tank");
	if (!wjson) {
		slog("Missing tank object in enemies json");
		sj_free(json);
		return NULL;
	}
	...
	if (!sj_object_get_value_as_float(wjson, "health", &health)) {
		slog("tank's health not initialized correctly.");
	}
	else {
		printf("tank's health: %.2f\n", health);
	}
...
void tank_pursue(Entity* self, Entity* target) {
	...
	Sint32 mx = 0, my = 0;
	if (!target || !self) {
		slog("Missing Enities for tank_pursue");
		return;
	}
...
void tank_draw(Entity* self) {
	...
	if (self->sprite) {
		...
		Color mouseColor = gfc_color8(255, 255, 0, 255);
